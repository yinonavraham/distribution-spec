...
The client SHOULD include a `Docker-Manifest-Tag` and / or `Docker-Manifest-Digest` headers indicating which manifest tag or digest is being pulled.
When pulling a manifest list, the `Docker-Manifest-Digest` header SHOULD include multiple values with all the manifest digests in the chain,
where the leftmost digest is of the first resolved manifest list, 
and the right most digest is of the last resolved manifest. 
These headers SHOULD be added to all requests throughout the pull process.
Registries MAY use these values as a hint when handling the pulled manifests and blobs.
...
The client SHOULD include a `Docker-Manifest-Tag` and / or `Docker-Manifest-Digest` headers indicating which manifest tag or digest is being pushed.
These headers SHOULD be added to all requests throughout the push process.
Registries MAY use these values as a hint when handling the pushed manifests and blobs.
